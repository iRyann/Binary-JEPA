...
\begin{tikzpicture}[
  ...
  tok/.style={
    draw=white, minimum width=0.58cm, minimum height=0.58cm,
  ...
  sgLbl/.style={font=\scriptsize, colTgt!80!black},
]
...
\foreach \i/\col/\lbl in {
  0/colCtx!25/{\scriptsize VEX},
  1/colCtx!25/{\scriptsize OP},
  2/gray!5/{\scriptsize API},
  3/gray!5/{\scriptsize JK},
...
\draw[arr, colCtx] (-3.4, 5.62) -- (ftheta.north);
\draw[arr, colTgt]  (3.4, 5.62) -- (fxi.north);
...
\node[sgLbl] at (4.95, 3.55) {stop-grad};
...
\node[repr, fill=colPred!8, draw=colPred]
  (zhat) at (0, -0.3) {$\hat{\bm{Z}} \in \mathbb{R}^{L \times d}$};
...
% conv1
\node[layer, fill=colCtx!15, draw=colCtx, right=1.5cm of emb]
  (c1) {\texttt{Conv1D}\\$k=5,\,p=2$\\$e \to h$\\\texttt{GELU}};
 
% conv2
\node[layer, fill=colCtx!20, draw=colCtx, right=1.5cm of c1]
  (c2) {\texttt{Conv1D}\\$k=5,\,p=2$\\$h \to h$\\\texttt{GELU}};
 
% conv3
\node[layer, fill=colCtx!28, draw=colCtx, right=1.5cm of c2]
  (c3) {\texttt{Conv1D}\\$k=3,\,p=1$\\$h \to d$};
 
% sortie
\node[draw=colCtx, fill=colCtx!8, rounded corners=3pt,
      minimum width=1.6cm, minimum height=0.55cm, right=1.5cm of c3]
...
% -- Légende frontières --
\draw[dashed, gray] (4*0.65+0.01, -0.15) -- (4*0.65+0.01, 2.9)
  node[above, font=\tiny] {$\lfloor\ell/3\rfloor$};
\draw[dashed, gray] (8*0.65+0.01, -0.15) -- (8*0.65+0.01, 2.9)
  node[above, font=\tiny] {$\lfloor 2\ell/3\rfloor$};
\draw[dashed, gray!50] (12*0.65+0.01, -0.15) -- (12*0.65+0.01, 2.9)
